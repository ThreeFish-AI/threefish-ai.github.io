...
我们知道训练神经网络模型的核心是以损失函数为基准来调整优化网络参数，使得网络的输出尽可能接近真实标签。在神经网络中，优化网络参数需要计算每个权重参数的梯度，不同的网络结构，计算梯度的方式和复杂度往往大不相同，有没有一种算法，即可以有效囊括所有类型的网络结构的梯度计算，又足以保证梯度计算的高效性？答案就是我们今天要讲的`误差逆传播`算法。
...
因为 $\frac{1}{\sum_{i=1}^{n} e^{a_i}}$ 分别与 n 项 $e^{a_i}$ 做乘法运算，逆传播计算局部梯度`求所有分支的和` - $t_k \sum_{i=1}^{n} e^{a_i}$，此时梯度为：
