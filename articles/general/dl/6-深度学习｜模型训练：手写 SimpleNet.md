...
| 成员                                                                                   | 类型 | 说明                                                                                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| params                                                                                 | 变量 | 存放 SimpleNet 网络权重参数与偏置参数： <br/>&nbsp;&nbsp;W1: 第 1 层网络的权重参数； <br/>&nbsp;&nbsp;b1: 第 1 层网络的偏置参数； <br/>&nbsp;&nbsp;W2: 第 2 层网络的权重参数； <br/>&nbsp;&nbsp;b2: 第 2 层网络的偏置参数。                                                                                                       |
| `__init__`(self, input_size, <br/>hidden_size, output_size, <br/>weight_init_std=0.01) | 方法 | SimpleNet 的初始化函数。 <br/>Args: <br/>&nbsp;&nbsp;input_size: 输入层（第 0 层）神经元个数（神经网络入参个数） <br/>&nbsp;&nbsp;hidden_size: 隐藏层（第 1 层）神经元个数 <br/>&nbsp;&nbsp;output_size: 输出层（第 2 层）神经元个数（神经网络出参个数） <br/>&nbsp;&nbsp;weight_init_std: 用于初始化权重参数的高斯分布的标准差。 |
| predict(self, x)                                                                       | 方法 | 推理函数，识别数字图像代表的数值。                                                                                                                                                                                                                                                                                                |
| loss(self, x, t)                                                                       | 方法 | 损失函数（交叉熵误差）                                                                                                                                                                                                                                                                                                            |
| numerical_gradient(self, x, t)                                                         | 方法 | 梯度函数（数值微分法）                                                                                                                                                                                                                                                                                                            |
...
此处需要特别注意**从 MNIST 数据集中取出的数据的类型处理**，图像数据需要进行归一化处理，标签数据则需要转换为 one-hot 表示，这些细节将决定模型的训练和推理是否会出现计算值溢出、参数是否会收敛等。
...
此处我们使用数值微分法实现的梯度计算仅供算法推导过程的演示，便于读者理解其数学过程，实际的训练中数值微分法的效率过于低下，是不可用的。笔者在 M1 Pro 机器上测试，每次迭代的耗时在 10s 左右，整个过程进行 10000 次迭代，约需要 27.7 hours，这是不可忍受的。我们在 GitHub 演示代码中提供了 `BP 算法`实现的梯度函数，采用 BP 算法进行训练，整个过程可以不超过 1min。关于 BP 算法，我们将在后续专属篇章中着重探讨。
...
我们可以使用如下代码画出训练过程中训练集和测试集的识别精度随训练轮次变化的图形：
